...
public:
	...
	void testRotations()
	{
		CPPUNIT_ASSERT_EQUAL( -r1, rot2_inverse(r1) );
		CPPUNIT_ASSERT_EQUAL( rot2_mult(r1,r1), rot2_mult( rot2_mult(r0,r0), r0 ) );
		CPPUNIT_ASSERT_EQUAL( rightOrthogonal(p0), rot2_map(r1,p0) );
		CPPUNIT_ASSERT_EQUAL( zero2, rot2_map(r0,zero2) );
		CPPUNIT_ASSERT_EQUAL( p0, rot2_map(rot2_id,p0) );
	}
	...
	void testTransformations()
	{
		move2 m0(r0,p0);
		move2 m1(r1,p1);
		CPPUNIT_ASSERT_EQUAL( p2 + p1, leftOrthogonal(p2) >> m1 );
		CPPUNIT_ASSERT_EQUAL( move2_id, inverse(inverse(m0)) );
		CPPUNIT_ASSERT_EQUAL( move2_id, compose(inverse(m0),m0) );
		CPPUNIT_ASSERT_EQUAL( p1, p1 >> m0 >> inverse(m0) );
		CPPUNIT_ASSERT_EQUAL( p0, zero2 >> m0 );
		CPPUNIT_ASSERT_EQUAL( p3, move2_id.map(p3) );
	}
};
